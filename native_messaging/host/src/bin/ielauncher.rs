...
fn recv_message() -> WebExtMessage {
    ...
    io::stdin().read_exact(&mut buffer).expect("Failed to read the message");
...
fn send_message(message: &str) {
    ...
    io::stdout().write_u32::<NativeEndian>(size as u32).expect("Failed to write the size");
    io::stdout().write_all(encoded_message.as_bytes()).expect("Failed to write the message");
...
fn main() {
    loop {
        match recv_message() {
            WebExtMessage::NavigationDetails { url, request_body, request_headers } => {
