...
COBA.exportSettings = function () {
  let aOld = COBA._getAllSettings(false);
  COBA.setOptions(true);
  let aCurrent = COBA._getAllSettings(false);
...
COBA.importSettings = function () {
  let aOld = COBA._getAllSettings(false);
  let [result, aList] = COBA._loadFromFile();
  if (!result || !aList) return;
...
COBA.restoreDefaultSettings = function () {
  let aOld = COBA._getAllSettings(false);
  let aDefault = COBA._getAllSettings(true);
...
COBA.setOptions = function (quiet) {
  ...
  filter = document.getElementById('filtercbx-official').checked;
  Services.prefs.setBoolPref('extensions.coba.official.filter', filter);
  let update = document.getElementById('autoUpdateFilter').checked;
...
COBA.addFilterRule = function (rule, enabled) {
  let idx = COBA.findRule(rule);
  let rules = document.getElementById('filterChilds');
  if (idx == -1) {
    let item = document.createElement('treeitem');
    let row = document.createElement('treerow');
    let c1 = document.createElement('treecell');
    let c2 = document.createElement('treecell');
...
COBA.addOfficialFilterRule = function (rule, enabled) {
  let rules = document.getElementById('filterChilds-official');
  let item = document.createElement('treeitem');
  let row = document.createElement('treerow');
  let c1 = document.createElement('treecell');
  let c2 = document.createElement('treecell');
...
COBA.initOfficialFilterList = function (def) {
  let list = COBA.getPrefOfficialFilterList(def);
  let rules = document.getElementById('filterChilds-official');
  while (rules.hasChildNodes())
    rules.removeChild(rules.firstChild);
  for (let i = 0; i < list.length; i++) {
...
COBA.initDialog = function () {
  ...
  let newurl = (window.arguments ? window.arguments[0] : ''); //get CurrentTab's URL
...
COBA.updateFilterStatus = function () {
  let en = document.getElementById('filtercbx').checked;
...
COBA.updateOfficialFilterStatus = function () {
  let en = document.getElementById('filtercbx-official').checked;
...
COBA.getFilterListString = function () {
  ...
  let count = filter.view.rowCount;

  for (let i = 0; i < count; i++) {
    let rule = filter.view.getCellText(i, filter.columns['columnRule']);
    let enabled = filter.view.getCellValue(i, filter.columns['columnEnabled']);
    let item = rule + (enabled == 'true' ? '' : '\b');
    list.push(item);
  }
  list.sort();
  return list.join(' ');
...
COBA.getOfficialFilterListString = function () {
  let list = [];
  let filter = document.getElementById('filterList-official');
  let count = filter.view.rowCount;

  for (let i = 0; i < count; i++) {
    let rule = filter.view.getCellText(i, filter.columns['columnRule-official']);
    let enabled = filter.view.getCellValue(i, filter.columns['columnEnabled-official']);
    let item = rule + (enabled == 'true' ? '' : '\b');
    list.push(item);
  }
  list.sort();
...
COBA.delSelected = function () {
  ...
  if (filter.view.selection.count > 0) {
    for (let i = rules.childNodes.length - 1; i >= 0; i--) {
...
COBA._getAllSettings = function (isDefault) {
  let prefix = 'extensions.coba.';
  let branch = (isDefault ? Services.prefs.getDefaultBranch('') : Services.prefs.getBranch(''));
  let preflist = branch.getChildList(prefix, {});
...
COBA._setAllSettings = function (aList) {
  ...
  for (let i = 1; i < aList.length; i++) {
    ...
    if (index > 0) {
      let name = aList[i].substring(0, index);
      let value = aList[i].substring(index + 1, aList[i].length);
